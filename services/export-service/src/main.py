...
# Database configuration - Try secrets manager first, fall back to environment variables
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("EXPORT_SERVICE_PORT", "8097"))
    ...
    if tls_enabled:
        ...
        logger.info(f"Starting export-service with TLS 1.3 on port {port}")
        uvicorn.run(app, host="0.0.0.0", port=port, ssl=ssl_context)
    else:
        logger.info(f"Starting export-service without TLS on port {port}")
