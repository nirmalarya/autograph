...
async def update_team_custom_role(
    role_name: str,
...
# ============================================================================
# CUSTOM ROLES API (Feature #530 - Enterprise)
# ============================================================================
...
class CustomRoleCreate(BaseModel):
    ...
    can_edit_own_diagrams: bool = True
    can_edit_all_diagrams: bool = False
    can_delete_own_diagrams: bool = True
    can_delete_all_diagrams: bool = False
    can_share_diagrams: bool = True
    can_comment: bool = True
    can_export: bool = True
    can_view_analytics: bool = False
    can_manage_team_settings: bool = False
...
class CustomRoleUpdate(BaseModel):
    ...
    can_remove_members: bool = None
    can_manage_roles: bool = None
    can_create_diagrams: bool = None
    can_edit_own_diagrams: bool = None
    can_edit_all_diagrams: bool = None
    can_delete_own_diagrams: bool = None
    can_delete_all_diagrams: bool = None
    can_share_diagrams: bool = None
    can_comment: bool = None
    can_export: bool = None
    can_view_analytics: bool = None
    can_manage_team_settings: bool = None


@app.post("/api/v1/teams/{team_id}/custom-roles", status_code=status.HTTP_201_CREATED)
async def create_team_custom_role(
    team_id: str,
    ...
    Only team admins can create custom roles.
    """
    try:
        from .models import Team, TeamMember
        ...
        if not team_member:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only team admins can create custom roles"
            )

        # Check if role name already exists for this team
        existing_role = db.query(CustomRole).filter(
            CustomRole.team_id == team_id,
            CustomRole.name == role_create.name
        ).first()

        if existing_role:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Role '{role_create.name}' already exists for this team"
            )
        ...
        custom_role = CustomRole(
            ...
            can_invite_members=role_create.can_invite_members,
            can_remove_members=role_create.can_remove_members,
            can_manage_roles=role_create.can_manage_roles,
            can_create_diagrams=role_create.can_create_diagrams,
            can_edit_own_diagrams=role_create.can_edit_own_diagrams,
            can_edit_all_diagrams=role_create.can_edit_all_diagrams,
            can_delete_own_diagrams=role_create.can_delete_own_diagrams,
            can_delete_all_diagrams=role_create.can_delete_all_diagrams,
            can_share_diagrams=role_create.can_share_diagrams,
            can_comment=role_create.can_comment,
            can_export=role_create.can_export,
            can_view_analytics=role_create.can_view_analytics,
            can_manage_team_settings=role_create.can_manage_team_settings,
            is_system_role=False,
            created_by=current_user.id
        )
        ...
        logger.info(f"Custom role created: {custom_role.id} for team {team_id}")

        return {
            "id": custom_role.id,
            "team_id": custom_role.team_id,
            "name": custom_role.name,
            "description": custom_role.description,
            "permissions": {
                "can_invite_members": custom_role.can_invite_members,
                "can_remove_members": custom_role.can_remove_members,
                "can_manage_roles": custom_role.can_manage_roles,
                "can_create_diagrams": custom_role.can_create_diagrams,
                "can_edit_own_diagrams": custom_role.can_edit_own_diagrams,
                "can_edit_all_diagrams": custom_role.can_edit_all_diagrams,
                "can_delete_own_diagrams": custom_role.can_delete_own_diagrams,
                "can_delete_all_diagrams": custom_role.can_delete_all_diagrams,
                "can_share_diagrams": custom_role.can_share_diagrams,
                "can_comment": custom_role.can_comment,
                "can_export": custom_role.can_export,
                "can_view_analytics": custom_role.can_view_analytics,
                "can_manage_team_settings": custom_role.can_manage_team_settings
            },
            "is_system_role": custom_role.is_system_role,
            "created_at": custom_role.created_at.isoformat(),
...
async def list_team_custom_roles(
    ...
    try:
        ...
        roles = db.query(CustomRole).filter(
            CustomRole.team_id == team_id
        ).order_by(CustomRole.created_at.desc()).all()
            ...
            "roles": [
                {
                    ...
                    "permissions": {
                        ...
                        "can_create_diagrams": role.can_create_diagrams,
                        "can_edit_own_diagrams": role.can_edit_own_diagrams,
                        "can_edit_all_diagrams": role.can_edit_all_diagrams,
                        "can_delete_own_diagrams": role.can_delete_own_diagrams,
                        "can_delete_all_diagrams": role.can_delete_all_diagrams,
                        "can_share_diagrams": role.can_share_diagrams,
                        "can_comment": role.can_comment,
                        "can_export": role.can_export,
                        "can_view_analytics": role.can_view_analytics,
                        "can_manage_team_settings": role.can_manage_team_settings
                    },
        ...
        logger.error(f"Failed to list custom roles: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to list custom roles"
        )
...
@app.get("/api/v1/teams/{team_id}/custom-roles/{role_id}")
async def get_team_custom_role(
    team_id: str,
    role_id: str,
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        logger.error(f"Failed to get custom role: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to get custom role"
        )
...
@app.put("/api/v1/teams/{team_id}/custom-roles/{role_id}")
async def update_team_custom_role(
    team_id: str,
    role_id: str,
    role_update: CustomRoleUpdate,
    ...
    try:
        ...
        if not team_member:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only team admins can update custom roles"
            )
        ...
        logger.info(f"Custom role updated: {role.id}")
        ...
        logger.error(f"Failed to update custom role: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to update custom role"
...
@app.delete("/api/v1/teams/{team_id}/custom-roles/{role_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_team_custom_role(
    team_id: str,
    role_id: str,
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        if not team_member:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Only team admins can delete custom roles"
            )
        ...
        members_with_role = db.query(TeamMember).filter(
            TeamMember.custom_role_id == role_id
        ).count()

        if members_with_role > 0:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail=f"Cannot delete role: {members_with_role} team members are using it"
            )
        ...
        logger.error(f"Failed to delete custom role: {str(e)}")
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to delete custom role"
