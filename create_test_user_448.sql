-- Create verified test user for feature #448
DO $$
DECLARE
    v_user_id VARCHAR(36) := 'test-user-448-' || substring(md5(random()::text) from 1 for 8);
    v_email VARCHAR(255) := 'test_user_448@example.com';
...
SELECT id, email, full_name, is_verified FROM users WHERE email = 'test_user_448@example.com';
