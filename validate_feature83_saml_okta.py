#!/usr/bin/env python3
"""
Validation script for Feature #83: SAML SSO with Okta

Tests:
1. Configure Okta as SAML provider
2. Enter Okta Entity ID and SSO URL
3. Upload Okta certificate
4. Click 'Sign in with Okta'
5. Redirect to Okta login
6. Authenticate with Okta
...
# Test data
TEST_ADMIN = {
    "email": f"admin_okta_test_{int(time.time())}@example.com",
...
def test_configure_saml_provider(admin_token: str) -> bool:
    """Test #1-3: Configure SAML SSO with Okta details."""
    print("\n1. Testing SAML Provider Configuration (Okta)...")

    # Okta configuration - using the correct API format
    saml_config = {
        "name": "okta",
        "enabled": True,
        "entity_id": "http://www.okta.com/exk1234567890",
        "sso_url": "https://dev-123456.okta.com/app/dev-123456_autograph_1/exk1234567890/sso/saml",
        "slo_url": "https://dev-123456.okta.com/app/dev-123456_autograph_1/exk1234567890/slo/saml",
        "x509_cert": "",
        "attribute_mapping": {
            "email": "email",
            "firstName": "firstName",
            "lastName": "lastName",
    ...
    print("   ✅ Okta SAML provider configured")
    return True
...
def test_list_saml_providers(admin_token: str) -> bool:
    ...
    okta_found = any(p.get("name") == "okta" for p in providers)

    if not okta_found:
        print(f"   ❌ Okta provider not in list")
        return False

    print(f"   ✅ Okta provider found in list ({len(providers)} total providers)")
    return True
...
def test_get_saml_provider(admin_token: str) -> bool:
    ...
    response = requests.get(
        f"{API_BASE}/admin/saml/providers/okta",
        headers={"Authorization": f"Bearer {admin_token}"},
        verify=VERIFY_SSL
    )
    ...
    expected_entity_id = "http://www.okta.com/exk1234567890"
    if config.get("entity_id") != expected_entity_id:
        print(f"   ❌ Entity ID mismatch: got {config.get('entity_id')}")
        return False

    sso_url = config.get("sso_url", "")
    if "okta.com" not in sso_url:
        print(f"   ❌ SSO URL doesn't contain okta.com: {sso_url}")
        return False

    print("   ✅ Okta configuration retrieved successfully")
    return True

def test_saml_login_endpoint(admin_token: str) -> bool:
    """Test #4-5: SAML login endpoint initiates redirect to Okta."""
    print("\n4. Testing SAML Login Endpoint (Okta)...")

    # Don't follow redirects - we want to see the redirect URL
    response = requests.get(
        f"{API_BASE}/auth/saml/login/okta",
    ...
    # Verify redirect is to Okta
    if "okta.com" not in redirect_url:
        print(f"   ❌ Redirect URL doesn't contain okta.com: {redirect_url}")
        return False
    ...
    print(f"   ✅ Redirects to Okta SSO ({len(redirect_url)} bytes)")
    return True
...
def test_saml_metadata_endpoint() -> bool:
    ...
    response = requests.get(
        f"{API_BASE}/auth/saml/metadata/okta",
        verify=VERIFY_SSL
    )
...
def main():
    ...
    print("=" * 70)
    print("Feature #83: SAML SSO with Okta - Validation")
    print("=" * 70)
    ...
    try:
        ...
        results.append(("Configure Okta SAML Provider", test_configure_saml_provider(admin_token)))
        ...
        if passed == total:
            print("\n🎉 Feature #83 (SAML SSO with Okta) - VALIDATED")
            return 0
        else:
            print(f"\n❌ Feature #83 - FAILED ({total - passed} tests failed)")
            return 1
